...
library(BART)
# post_mu_samples_bart_func.R
post_mu_samples_bart = function(m1X_samples = NULL, m0X_samples = NULL, Xtest = NULL, Ytest = NULL, ind1_test = NULL, ind0_test = NULL, ps_samples = NULL, phat = NULL, M = NULL){
  m1X_samples = as.matrix(m1X_samples)
  m0X_samples = as.matrix(m0X_samples)
  ...
  ##########################################################################
  m1_X_t1 = m1X_samples[ind1_test, ] # n1xM matrix each column is m_1(X)
  #obtain lambda tilde
  ...
  ##########################################################################
  m0_X_t0 = m0X_samples[ind0_test, ]  # n0xM matrix each column is m_0(X)
  #obtain lambda tilde
  ...
  #### obtaining samples for mutilde
  mX = m1X_samples - m0X_samples# n many m1(X)
  df_m = n - 1 
  mX_bar = colMeans(mX)
  ...
  remove(mX,mX_bar, eta_m, s2_m, mutilde_samples)
  gc(verbose = getOption("verbose"), reset = FALSE, full = TRUE)

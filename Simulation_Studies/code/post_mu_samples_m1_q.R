...
# post_mu_samples_m1_q.R
post_mu_samples_m1_q_func = function(m1samples = NULL, m0samples = NULL, ps_samples = NULL, phat = NULL, Xtest = NULL, Ytest = NULL, trt_test = NULL, ind1_test = NULL, ind0_test = NULL, M = NULL){
  ...
  X1_t = as.matrix(Xtest[ind1_test, ]) # test data on T = 1
  X1 = cbind(1, X1_t) # test data on T = 1
  W = cbind(1, X1_t, X1_t^2)
  # control data
  ...
  ##########################################################################
  m1_X_t1 = W %*% m1samples # n1xM each column is m_1(X)
  #obtain lambda tilde
  ...
  #m_samples = m1samples_l - m0samples
  mX = cbind(Xtilde, Xtest^2) %*% m1samples -  Xtilde%*% m0samples# n many m1(X)
  df_m = n - 1 
  mX_bar = colMeans(mX)
